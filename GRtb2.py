...
class MyFrame(wx.Frame):
    ...
    def PlotEve(self, event):  # wxGlade: MyFrame.<event_handler>
        ...
        try:
            ...
            self.Figure2.draw()
            fig2R = round(sum(sum(fig2))/sum(sum(GS.EE)),4)
            self.EpidermisRatio.SetValue(str(fig2R))
        except:
            pass

        try:
            fig3 = np.multiply(fig, GS.VV)
            ...
            self.Figure3.draw()
            fig3R = round(sum(sum(fig3)) / sum(sum(GS.VV)), 4)
            self.VascularBundleRatio.SetValue(str(fig3R))
        except:
            pass

        try:
            fig7 = np.multiply(fig, GS.SS)
            ...
            self.Figure7.draw()
            fig7R = round(sum(sum(fig7)) / sum(sum(GS.SS)), 4)
            self.SclerenchymaRatio.SetValue(str(fig7R))
        except:
            pass

        try:
            fig8 = np.multiply(fig, GS.PP)
            ...
            self.Figure8.draw()
            fig8R = round(sum(sum(fig8)) / sum(sum(GS.PP)), 4)
