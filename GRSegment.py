...
class MyFrame(wx.Frame):
    ...
    def ExtracteWatershed(self, event):  # wxGlade: MyFrame.<event_handler>
        ...
        if self.checkbox_11.GetValue():
            GS.thicky = int(self.text_ctrl_2.GetValue())  # 皮层厚度
            EE = np.zeros([m, n])
            for ii in np.arange(0, m, 1):
                sum = 0
                for jj in np.arange(0, n, 1):
            ...
            GS.EE = np.copy(EE)
            GS.AreaEE = np.sum(EE)
            self.text_ctrl_EE.SetValue(str(GS.AreaEE))
        ...
        GS.SS = np.copy(SS)
        GS.AreaSS = np.sum(SS)
        self.text_ctrl_SS.SetValue(str(GS.AreaSS))
        ...
        GS.VV = np.copy(VV)
        GS.AreaVV = np.sum(VV)
        self.text_ctrl_VV.SetValue(str(GS.AreaVV))
        ...
        if self.checkbox_14.GetValue():
            ...
            GS.PP = np.copy(PP)
            GS.AreaPP = np.sum(PP)
            self.text_ctrl_PP.SetValue(str(GS.AreaPP))
    ...
    def EEpreview(self, event):  # wxGlade: MyFrame.<event_handler>
        ...
        self.frame.setParent(self, 'EE', GS.EE, self.Figure1, self.matplotlib_axes1, 'gray', self.text_ctrl_EE)
        self.Hide()
        self.frame.Show()
        self.frame.Center()
    ...
    def VVpreview(self, event):  # wxGlade: MyFrame.<event_handler>
        ...
        # setParent是GRPurge的方法之一
        # setParent(self, parent, parentName, data, figure, axes)
        self.frame.setParent(self, 'VV', GS.VV, self.Figure3, self.matplotlib_axes3, 'gray', self.text_ctrl_VV)
        self.Hide()
        self.frame.Show()
        self.frame.Center()
        # 进行一次缩放，各控件(尤其是Canvas)完整显示
    ...
    def SSpreview(self, event):  # wxGlade: MyFrame.<event_handler>
        ...
        # setParent是GRPurge的方法之一
        # setParent(self, parent, parentName, data, figure, axes)
        self.frame.setParent(self, 'SS', GS.SS, self.Figure2, self.matplotlib_axes2, 'gray', self.text_ctrl_SS)
        self.Hide()
        self.frame.Show()
        self.frame.Center()
        # 进行一次缩放，各控件(尤其是Canvas)完整显示
    ...
    def PPpreview(self, event):  # wxGlade: MyFrame.<event_handler>
        ...
        # setParent是GRPurge的方法之一
        # setParent(self, parent, parentName, data, figure, axes)
        self.frame.setParent(self, 'PP', GS.PP, self.Figure4, self.matplotlib_axes4, 'gray', self.text_ctrl_PP)
        self.Hide()
        self.frame.Show()
        self.frame.Center()
        # 进行一次缩放，各控件(尤其是Canvas)完整显示
    ...
    def ConfirmSave(self, event):  # wxGlade: MyFrame.<event_handler>
        ...
        #try:
            ...
            #dlg.SetFilterIndex(0)
                ...
                #path = dlg.GetPath()
                #print(path)
                #plt.imshow(GS.VisimageO, cmap='jet')
        ...
        try:
            ...
            if dlg.ShowModal() == wx.ID_OK:
                path = dlg.GetPath()
                print(path)
                plt.imshow(GS.visGray, cmap='jet')
                plt.rcParams['savefig.dpi'] = 300  # 1920*1440 的图片
                plt.xticks([])
                plt.yticks([])
                plt.savefig(path)
        ...
        if dlg.ShowModal() == wx.ID_OK:
            path = dlg.GetPath()
            print(path)
            np.savez(path, EqualizedImage=GS.visGray, dataList=list1, dataName=list2, dataArea=list3)
