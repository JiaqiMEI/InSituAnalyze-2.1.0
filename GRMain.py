...
class MyFrame(wx.Frame):
    ...
    def HyperImport(self, event):  # wxGlade: GRMain.<event_handler>
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if (len(list(Hyper.keys())) == 1 and len(Hyper[list(Hyper.keys())[0]].shape) != 3) or (
                    len(list(Hyper.keys())) == 4 and len(Hyper[list(Hyper.keys())[3]].shape) != 3):
                ...
                if dlg.ShowModal() == wx.ID_OK:
                    ...
                    if (len(list(Wave.keys())) == 1 and len(Wave[list(Wave.keys())[0]].shape) != 2) or (
                            len(list(Wave.keys())) == 4 and len(Wave[list(Wave.keys())[3]].shape) != 2):
                        ...
                        k = cv2.resize(G.Visimage, (G.n, G.m), interpolation=cv2.INTER_CUBIC)
                        G.Visimage = np.copy(k)
                        print(G.Visimage.shape)
    ...
    def fgExtract(self, event):  # wxGlade: GRMain.<event_handler>
        ...
        if G.Visimage.size == 0:
            ...
            GS.grayimage = np.copy(grayImage)
            ...
            self.frame.SetSize((1000, 750))

            self.frame.matplotlib_axes7.imshow(G.Visimage, cmap='jet')
    ...
    def TissueExtractEve(self, event):  # wxGlade: GRMain.<event_handler>
        ...
        if G.Visimage.size == 0:
            ...
            # 传递去背景的伪彩色灰度图像GS.fore至GRPurge，返回值为去背景和去噪点的伪彩色灰度图像GS.foreP
            GS.grayimage = np.copy(GS.foreP)
            # 偶尔数据类型不对，统一校正一次uint8
            GS.grayimage = GS.grayimage.astype(np.uint8)

            # 求G.foreP二值图以对G.Visimage去背景
            fore2 = np.copy(GS.foreP)
            fore2[fore2 != 0] = 1
            GS.fore2 = np.copy(fore2)
            for ii in np.arange(0, 3, 1):
                G.Visimage[:, :, ii] = np.multiply(G.Visimage[:, :, ii], GS.fore2)
            # BGR转YUV，校正亮度分量，再转回来
            GS.VisimageYUV = cv2.cvtColor(G.Visimage, cv2.COLOR_BGR2YCrCb)
            GS.VisimageYUV[0] = cv2.equalizeHist(GS.VisimageYUV[0])
            # 亮度均衡化后的BGR
            GS.visNew = cv2.cvtColor(GS.VisimageYUV, cv2.COLOR_YCrCb2BGR)
            # 均衡化后的灰度
            GS.dstGray = cv2.equalizeHist(GS.grayimage)
            # 用于后续分割的数据
            GS.visGray = np.dstack([GS.visNew, GS.dstGray])
            ...
            # 在分割界面绘制均衡化后的G.Visimage == GS.visGray
            self.frame.matplotlib_axes7.imshow(GS.visGray, cmap='brg')
            self.frame.matplotlib_axes7.set_xticks([])
            self.frame.matplotlib_axes7.set_yticks([])
            self.frame.Figure7.draw()
